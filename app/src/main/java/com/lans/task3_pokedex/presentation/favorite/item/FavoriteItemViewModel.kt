...
class FavoriteItemViewModel @Inject constructor(
    ...
    private val _state = MutableStateFlow(FavoriteItemUIState())
    val state: Flow<FavoriteItemUIState> = _state
    ...
    private fun loadAllFavoriteItem() {
        viewModelScope.launch {
            ...
            favoriteRepository.getAllFavoriteItemDetail(userId).collect { result ->
                when (result) {
                    is Resource.Success -> {
